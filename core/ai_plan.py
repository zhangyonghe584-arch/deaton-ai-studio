...
Use 2-4 bullet points beneath each non-title heading. LOCAL DESIGN NOTES must assign content across exactly five output images and describe three genuinely different layout directions for the local renderer. If fewer than five photos are supplied, explicitly mark which output images reuse an available photo or rely on supplied case facts; never invent a missing photo. The logo area must contain only the supplied logo; never add brand text, step labels, STEP, image numbers, case IDs, or placeholder digits. Preserve every source photo without cropping important vehicle or instrument information; use centered, non-cropping placement. Keep the plan factual and inexpensive to execute locally. The plan is an execution input, not a final answer: it will be saved locally and passed to the Pillow renderer to generate the five images."""
